...
class AirbnbSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        with gzip.open(Path.cwd() /'To_Scrape_v2.csv.gz', 'rt',"encoding = 'utf-8'") as read_obj:
            ...
            for i in range(int(self.start),int(self.end)):
                ...
                data_dict['listing_id'] = str(home_list[i][0])
                data_dict['reviews_count'] = str(home_list[i][1])
               
                yield scrapy.Request(url=review_url, callback=self.parse_reviews,meta=data_dict) 
    ...
    def parse_reviews(self, response):
        ...
        metadata_reviews_count = data.get('metadata').get('reviews_count')
        
        data_dict = collections.defaultdict(dict) # Create Dictionary to put all currently available fields in
        
        data_dict['listing_id'] = str(response.meta['listing_id'])
        data_dict['reviews_count'] = str(response.meta['reviews_count'])
        
        if(metadata_reviews_count > int(data_dict['reviews_count'])):
            
            data_dict['reviews_count'] = str(metadata_reviews_count)
            review_url = 'https://www.airbnb.com/api/v2/homes_pdp_reviews?&currency=USD&key=d306zoyjsyarp7ifhu67rjxn52tv0t20&locale=en&listing_id='+str(data_dict['listing_id'])+'&_format=for_p3&limit='+str(metadata_reviews_count)+'&offset=0&order=language_country'
            
            yield scrapy.Request(url=review_url, callback=self.parse_reviews,meta=data_dict) 
        
        elif(metadata_reviews_count <= int(data_dict['reviews_count'])):
            
            listing = AirbnbScraperItem()
            listing['listing_id'] = str(response.meta['listing_id'])
            listing['reviews_count'] = str(metadata_reviews_count)
            ...
            src = Path(r'D:\Airbnb\Review_Jsons')
